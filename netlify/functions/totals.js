...
export async function handler(event) {
  ...
  try {
    ...
    for (const page of res.results) { const date = page.properties?.["Date"]?.date?.start; const minutes = page.properties?.["Minutes"]?.number || 0; const key = keyFromISO(date); if (!key) continue; if (!days[key]) days[key] = { totalMs: 0 }; days[key].totalMs += minutes * 60000; }
